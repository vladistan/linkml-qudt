...
@app.cell
def _(mo):
    include_ancestors = mo.ui.checkbox(value=True, label="Include ancestor classes (is_a chain)")
    include_mixins = mo.ui.checkbox(value=True, label="Include mixin classes")
    include_slot_ranges = mo.ui.checkbox(value=True, label="Recursively include classes from slot ranges")
...
@app.cell
def _(class_selector, get_class_dependencies, include_ancestors, include_mixins, include_slot_ranges, sv):
    ...
    if selected_classes:
        ...
        dependencies = {"classes": set(), "slots": set(), "enums": set(), "types": set()}
...
def _(dependencies, mo):
    ...
    mo.accordion({
        ...
        "Enums to include": mo.ui.table(
            ...
            selection=None,
        ) if dependencies["enums"] else mo.md("*No enums*"),
        "Types to include": mo.ui.table(
            data=[{"Type": t} for t in sorted(dependencies["types"])],
            selection=None,
        ) if dependencies["types"] else mo.md("*No custom types*"),
    })
    return (classes_list,)
...
def _(mo):
    ...
    fix_qudt_prefix = mo.ui.checkbox(
        value=True,
        label="Fix QUDT prefix to http://qudt.org/schema/qudt/"
    )

    mo.vstack([remove_base_classes, remove_standard_types, rename_common_slots, fix_qudt_prefix])
    return remove_base_classes, remove_standard_types, rename_common_slots, fix_qudt_prefix
...
def _(SchemaDefinition, copy):
    def create_subset_schema(sv, dependencies: dict, schema_name: str = "qudt_subset") -> SchemaDefinition:
        ...
        if fix_qudt_prefix and 'prefixes' in schema:
            if 'qudt' in schema['prefixes']:
                ...
                if isinstance(old_val, dict):
                    if old_val.get('prefix_reference') != 'http://qudt.org/schema/qudt/':
                        old_ref = old_val.get('prefix_reference', 'unknown')
                        schema['prefixes']['qudt'] = 'http://qudt.org/schema/qudt/'
                        changes.append(f"Fixed qudt prefix: {old_ref} -> http://qudt.org/schema/qudt/")
                elif old_val != 'http://qudt.org/schema/qudt/':
                    schema['prefixes']['qudt'] = 'http://qudt.org/schema/qudt/'
                    changes.append(f"Fixed qudt prefix: {old_val} -> http://qudt.org/schema/qudt/")
        ...
        if removed_classes and 'classes' in schema:
            for cls_name, cls_def in schema['classes'].items():
                ...
                if cls_def.get('is_a') in removed_classes:
                    ...
                    changes.append(f"Changed {cls_name}.is_a from {old_parent} to Thing")

                # Fix mixin references
                if 'mixins' in cls_def:
                    new_mixins = [m for m in cls_def['mixins'] if m not in removed_classes]
                    if len(new_mixins) != len(cls_def['mixins']):
                        removed = set(cls_def['mixins']) - set(new_mixins)
                        changes.append(f"Removed mixins from {cls_name}: {', '.join(removed)}")
                ...
                if 'slot_usage' in cls_def:
                    for slot_name, usage in cls_def['slot_usage'].items():
                        if usage.get('range') in removed_classes:
                            ...
                            del usage['range']
                            changes.append(f"Removed range override {old_range} from {cls_name}.{slot_name}")
        ...
        if rename_common_slots:
            ...
            if slot_renames and 'classes' in schema:
                for cls_name, cls_def in schema['classes'].items():
                    # Update slots list
                    if 'slots' in cls_def:
                        cls_def['slots'] = [slot_renames.get(s, s) for s in cls_def['slots']]
...
def _(
    ...
    def save_schema_with_fixes():
        ...
        with open(output_path, "w") as f:
            pyyaml.dump(schema_dict, f, default_flow_style=False, sort_keys=False, allow_unicode=True)
...
@app.cell
def _(Path, SchemaView, get_validation_result, mo, output_path_input, set_validation_result):
    def validate_subset():
        output_path = Path(output_path_input.value)
    ...
    mo.vstack([validate_button, mo.md(_validation_result) if _validation_result else mo.md("")])
